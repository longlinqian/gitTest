第一次 获取远端代码 git clone "path"
每次提交前先同步服务器代码 :git pull 远端：本地   例子 :git pull origin master:master

开发；
在本地自己的分支开发:  新建分之: git checkout -b feature/linlong1  (feature/linlong1)是分支名
开发完成后需提交分之，并和：
git checkout -b feature/demo1   //新建分支，将改动转移到新建分支
git add --all 
git commit -m"说明"   本地暂存改动
git checkout release //release 为开发分支
git pull origin release: release  //获取远端最新代码
git merge feature/demo1  //将本地改动与远端最新代码合并
git push origin release:release   //同步远端代码

有冲突时  merge之后有冲突   解决冲突
然后 git add --all
    git commit -m""
    git push 
